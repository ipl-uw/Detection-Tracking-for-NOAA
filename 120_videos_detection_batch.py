...
def cmp_only_frame_id(s1,s2):
    ...
    s1 = int(s1.split('_')[-1].split('.')[0])
    s2 = int(s2.split('_')[-1].split('.')[0])
...
model = build_model(cfg) # returns a torch.nn.Mod
# ule
...
parser.add_argument('--result_save_dir', type=str,
                    default="./detection_result_"+str(len(thing_classes))+'_things_120_videos',
                    help='It is the folder of detection result csv file')

parser.add_argument('--folder_2021', type=str,
                    default='./data/Middleton_IP_2021',
                    help='It is the folder of all 120 rail data')

args = parser.parse_args()

rail_data_paths = sorted(os.listdir(args.folder_2021))




existing = os.listdir(args.result_save_dir)



for j, rail_data_path in tqdm(enumerate(rail_data_paths)):
    if 'mp4' in rail_data_path:
        continue
    if rail_data_path in existing:
        continue
    print('Detecting in folder:', rail_data_path)

    result_save_dir = os.path.join(args.result_save_dir, rail_data_path)
    ...
    BATCH_SIZE=16
    image_names = sorted([f for f in os.listdir(os.path.join(args.folder_2021,rail_data_path)) if f!="Thumbs.db"], key=functools.cmp_to_key(cmp_only_frame_id))
    dataset = Fish_Rail_Img(data_path = os.path.join(args.folder_2021,rail_data_path),img_names=image_names, cfg=cfg)
    ...
    f.close()
    print('Detect %d frames with objects in haul %s'%(num, rail_data_path))