...
def load_mot(detections, nms_overlap_thresh=None, with_classes=True, nms_per_class=False):
    ...
    all_frame_name = R[np.sort(idx)]
    


    print('loading csv detections...')
    for frame_name in tqdm(all_frame_name): #select detections in the same frame

        idx = raw[:, 0]== frame_name

        scores = raw[idx, 5].astype(float)



        if scores[0] == 0: # means no any detections in this frame
            bbox = np.array([['','','','']])
            classes = ['']
        else: # means there are detections in this frame
            bbox = raw[idx,1:5].astype(float)
            # bbox -=1
            classes = raw[idx, 6]

            if nms_overlap_thresh:
                bbox, scores, classes = nms(bbox, scores, nms_overlap_thresh, classes)

        dets = []
        for bb, s, c in zip(bbox, scores, classes):
            dets.append({'bbox': (bb[0], bb[1], bb[2], bb[3]), 'score': s, 'class': c,'frame_name': frame_name})
        data.append(dets)
    print('loading csv detections... Done!')
...
def save_to_csv(out_path, tracks, all_frame_name, fmt='motchallenge'):
    """
    ...
    with open(out_path+'/tracking_result.csv', "w") as ofile:
        ...
        for track in tracks:
            ...
            for i, bbox in enumerate(track['bboxes']):
                ...
                if fmt =='fish':
                    row = {'track id': id_,
                           'frame': all_frame_name[track['start_frame'] -1  + i], ##track['start_frame'] begins from 1 in viou_tracker.py
                           'xmin': bbox[0] + 1,
                           'ymin': bbox[1] + 1,
                           'xmax': bbox[2] + 1,
                           'ymax': bbox[3] + 1,
                           'score': track['frame_score'][i],
                           'cls': track['class']}
