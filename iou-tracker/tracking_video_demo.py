...
def load_mot_all_frames(detections, all_frame_names):
    ...
    for frame_name in tqdm(all_frame_names):
        ...
        for bb, s, c, id in zip(bbox, scores, classes, track_ids):
            dets.append({'id': id, 'bbox': (bb[0], bb[1], bb[2], bb[3]), 'score': s, 'class': c,'frame_name': frame_name})
        data.append(dets)
...
def save_as_one_video(image_path, save_dir, data,fps, box_thickness, txt_thickness, fontScale):
    ...
    for detections_frame in tqdm(data):
        ...
        for det in detections_frame:
            ...
            cv2.putText(img, 'fish', (xmin, ymax + int((ymax-ymin)*0.2)), cv2.FONT_HERSHEY_COMPLEX, fontScale, color, txt_thickness)
            cv2.putText(img, str(np.round(det['score'],3)), (xmax - int((xmax-xmin)*0.2), ymax + int((ymax-ymin)*0.2)), cv2.FONT_HERSHEY_COMPLEX, fontScale, color, txt_thickness)
            cv2.putText(img, 'id: '+str(int(det['id'])), (xmin, ymin - int((ymax -ymin)*0.2)), cv2.FONT_HERSHEY_COMPLEX, fontScale, color,txt_thickness)
            # 写入视频
        videoWriter.write(img)
...
def save_as_videos(image_path, save_dir, track_dict,fps, box_thickness, txt_thickness, fontScale):
    ...
    for track_id in tqdm(track_dict):
        ...
        for each_frame in each_track:
            ...
            cv2.rectangle(img, (xmin, ymin),(xmax, ymax), color, box_thickness)
            cv2.putText(img, each_frame['class'], (xmin, ymax + int((ymax-ymin)*0.2)), cv2.FONT_HERSHEY_COMPLEX, fontScale, color,txt_thickness)
            cv2.putText(img, each_frame['conf'][:4], (xmax - int((xmax-xmin)*0.2), ymax + int((ymax-ymin)*0.2)), cv2.FONT_HERSHEY_COMPLEX, fontScale, color, txt_thickness)
            cv2.putText(img, txt, (int(size[0]/8), int(size[1]/8)), cv2.FONT_HERSHEY_COMPLEX, fontScale, color, txt_thickness)
            #写入视频
            videoWriter.write(img)
...
## huber regression
file_path = '/home/jiemei/Documents/rail_detection/tracking_result_5_things/Predator_2018/20180824T162340-0800/tracking_result.csv'
frame_path = '/run/user/1000/gvfs/smb-share:server=ipl-noaa.local,share=homes/rail/Predator_2018/20180824T162340-0800/GO-2400C-PGE+09-88-35'



det_ROI = np.load(file_path.replace(file_path.split('/')[-1], 'det_ROI.npy'))

all_frame_names = np.load(file_path.replace('tracking_result.csv', 'detection_result_with_blank_frames-all_frame_names.npy'), allow_pickle=True).tolist()
data = load_mot_all_frames(file_path, all_frame_names)
if len(data) == 0:
    print('no tracks in this folder! no need huber regression.')
    embed()
...
save_as_videos(frame_path, save_dir, track_dict_huber,fps=20, box_thickness=3, txt_thickness=3, fontScale=1)# each track will be saves as one video
